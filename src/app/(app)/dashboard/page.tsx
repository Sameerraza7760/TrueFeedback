...
const page = () => {
  ...
  const messagesStatus = watch("messagesStatus");

  const fetchMessagesStatus = useCallback(async () => {
  ...
  const handleSwitchChange = async () => {
    try {
      const response = await axios.post<ApiResponse>("/api/accept-messages", {
        acceptMessages: !messagesStatus,
      });
      setValue("messagesStatus", !messagesStatus);
      toast({
    ...
    <div className="my-8 mx-4 md:mx-8 lg:mx-auto p-6 bg-white rounded w-full max-w-6xl">
      ...
      <div className="mb-4">
        <Switch
          ...
          checked={messagesStatus}
          onCheckedChange={handleSwitchChange}
          disabled={isSwitchLoading}
        />
        <span className="ml-2">
          Accept Messages: {messagesStatus ? "On" : "Off"}
        </span>
      </div>
      <Separator />
